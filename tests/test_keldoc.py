...
def test_keldoc_parse_center():
    ...
    assert motives == json.loads(
        Path("tests", "fixtures", "keldoc", "center1-motives.json").read_text()
    )

    # Find first availability date
    date, _ = test_center_1.find_first_availability("2020-04-04", "2020-04-05")
    assert not date
...
def test_keldoc_missing_params():
    ...
    center1_redirect = (
        "https://vaccination-covid.keldoc.com/redirect/?dom=foo&user=ok&specialty=no"
    )
...
def test_keldoc_filters():
    ...
    assert is_specialty_relevant(
        {
            "id": 1,
            "name": "Vaccination contre la COVID",
            "skills": [{"name": "Centre de vaccination COVID-19"}],
        }
    )
    assert not is_specialty_relevant(
        {"id": 1, "name": "Vaccination contre la COVID", "skills": [{"id": 123}]}
    )
