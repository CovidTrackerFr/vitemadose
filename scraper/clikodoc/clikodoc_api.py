...
def get_next_available_date(
    doctor_id: str, type_id: str, client: httpx.Client = DEFAULT_CLIENT, request: ScraperRequest = None, token: Optional[str] = None
) -> Optional[str]:
    url = CLIKODOC_API.get(
        "next_available_date", "https://www.clikodoc.com/foreapicntrl/nextavaildate?module=patients"
    )
    payload = {"doctorid": doctor_id, "typeid": type_id}
    headers = {
        "Content-Type": "application/x-www-form-urlencoded",
        "X-CSRF-TOKEN": token,
        "X-Requested-With": "XMLHttpRequest",
...
def get_slots(
    ...
    payload = {
        ...
        "to": end_date.isoformat(),  # ignored, always gives 3 next days with availabilities from start_date
        "slotfor": "next",
        "work_location_id": 0,
