    ...
    with open("data/output/mapharma-centers.json") as json_file:
        ...
        for zip in mapharma.keys():
            for profile in mapharma[zip]:
                id = profile['id']
                centre = {'gid': f'{zip}-{id}', 'rdv_site_web': profile['url'],
                          'com_insee': departementUtils.cp_to_insee(zip), 'nom': profile['name'],
                          "location": {"zip": zip}, "address": profile['address'], 'iterator': 'mapharma',
                          'type': DRUG_STORE}
                yield centre
