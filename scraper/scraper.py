...
from .departements import import_departements
...
def scrape():
    with Pool(POOL_SIZE) as pool:
        centres_cherchés = pool.imap_unordered(
            cherche_prochain_rdv_dans_centre, centre_iterator(), 1
        )
        compte_centres, compte_centres_avec_dispo, compte_bloqués = export_data(
            centres_cherchés
        )
        logger.info(
            f"{compte_centres_avec_dispo} centres de vaccination ont des disponibilités sur {compte_centres} scannés"
        )
        if compte_centres_avec_dispo == 0:
            logger.error(
                "Aucune disponibilité n'a été trouvée sur aucun centre, c'est probablement une erreur"
            )
            exit(code=1)

        if compte_bloqués > 10:
            logger.error(
                "Notre IP a été bloquée par le CDN Doctolib plus de 10 fois.\
                     Pour éviter de pousser des données erronées, on s'arrête ici"
            )
            exit(code=2)
...
def fix_scrap_urls(url):
    ...
    if url.startswith("https://www.keldoc.com/"):
        url = url.replace(
            "https://www.keldoc.com/", "https://vaccination-covid.keldoc.com/"
        )
    # Clean Doctolib
    if url.startswith("https://partners.doctolib.fr") or url.startswith(
        ...
        for query_name in query:
            if (
                query_name.startswith("highlight")
                or query_name == "enable_cookies_consent"
            ):
