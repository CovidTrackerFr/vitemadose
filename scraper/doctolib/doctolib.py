...
if not all(
    i <= (DOCTOLIB_SLOT_LIMIT * (DOCTOLIB_ITERATIONS + 1)) for i in INTERVAL_SPLIT_DAYS
):
...
class DoctolibSlots:
    ...
    def __init__(
        self, client: httpx.Client = None, cooldown_interval=WAIT_SECONDS_AFTER_REQUEST
    ) -> None:
        ...
        for visit_motive_id in visit_motive_ids:
            ...
            if agenda_ids != [] and practice_ids != []:
                ...
                for i in range(DOCTOLIB_ITERATIONS):
                    ...
                    updated_dict = dict(
                        Counter(request.appointment_schedules)
                        + Counter(count_next_appt)
                    )
            ...
            for interval in INTERVAL_SPLIT_DAYS:
                if start_date <= append_date_days(start_date_original, interval):
                    if availability.get("date"):
                        if availability.get("date") <= append_date_days(
                            start_date_original, interval
                        ):
                            next_appointment_timetables[f"{interval}_days"] += len(
...
def set_doctolib_center_internal_id(request: ScraperRequest, data: dict, practice_ids):
    ...
    practices = (
        "-".join(str(x) for x in practice_ids) if practice_ids is not None else ""
    )
...
def link_practice_ids(practice_ids: list, rdata: dict):
    if not practice_ids:
        return practice_ids
    places = rdata.get("places")
    if not places:
    ...
    for place in places:
        ...
        if int(re.findall(r"\d+", place_id)[0]) == int(practice_ids[0]):
            ...
            if not int(place.get("practice_ids", [])[0]) in practice_ids:
                practice_ids.append(int(place.get("practice_ids", [])[0]))
            base_place = place
            break
    if not base_place:
        return place_ids
    for place in places:
        ...
        if place.get("address") == base_place.get("address"):  # Tideous check
            practice_ids.append(int(re.findall(r"\d+", place.get("id"))[0]))
    return practice_ids
...
def _find_visit_motive_id(data: dict, visit_motive_category_id: list = None):
    ...
    for visit_motive in data.get("data", {}).get("visit_motives", []):
        ...
        if (
            "allow_new_patients" in visit_motive
            and not visit_motive["allow_new_patients"]
        ):
            continue
        ...
        if (
            ...
            relevant_motives[visit_motive["id"]] = get_vaccine_name(
                visit_motive["name"]
            )
...
def _find_agenda_and_practice_ids(
    ...
    for agenda in data["data"]["agendas"]:
        ...
        for pratice_id_agenda, visit_motive_list_agenda in agenda[
